...
import { getDependencies, getUsedBy, formatResourceLabel } from '@/lib/generate/deps';
import type { FieldDef, ReferenceValue } from '@/lib/schema/types';
import { envScope, environmentById, sharedScope, tierShortLabel } from '@/lib/schema/environments';
import {
	HUB_OWNERSHIP_COPY,
		...
		return (
			<Card className="p-8 flex items-center justify-center h-full min-h-[280px]">
				<div className="text-center max-w-sm">
					...
					<h3 className="text-base font-semibold text-slate-800 dark:text-slate-100 mb-1">Select a resource</h3>
					<p className="text-sm text-slate-500">
						Choose one from the List or Graph view, or add from the catalogue. Selection is shared across views. Use
	...
	function renderField(field: FieldDef) {
		...
		if (resource!.useExisting && field.existingKey) {
			return (
				<div key={field.key}>
					<Label
						htmlFor={`ex-${field.key}`}
						required={field.required}>
						{field.label} <Badge tone="amber">existing</Badge>
					</Label>
					<TextInput
						id={`ex-${field.key}`}
						value={String(
							resource!.existingValues[field.key] ??
								(typeof resource!.values[field.key] === 'string' ? resource!.values[field.key] : '') ??
								'',
						)}
						onChange={(e) => updateExistingValue(resource!.id, field.key, e.target.value)}
						placeholder={field.placeholder}
					/>
					<Hint>Existing id/name used to look up this resource in Azure (not managed by this project).</Hint>
				</div>
			);
		}
		...
		if (field.type === 'reference') {
			return (
				<ReferencePicker
					...
					onChange={(v: ReferenceValue | undefined) => updateResourceValue(resource!.id, field.key, v)}
				...
				<div key={field.key}>
					...
					<SelectInput
						...
						onChange={(e) => {
							...
							if (resource!.type === 'azurerm_private_endpoint' && field.key === 'subresource_names') {
								const suggested =
									next === 'vault'
										? 'psc-kv'
										: next === 'sqlServer'
											...
											: next === 'registry'
												? 'psc-acr'
												: 'psc';
								const cur = String(resource!.values.private_connection_name ?? '');
								if (!cur || cur === 'psc' || cur === 'psc-acr' || cur === 'psc-kv' || cur === 'psc-sql') {
									updateResourceValue(resource!.id, 'private_connection_name', suggested);
								}
							}
						}}>
						{(field.options ?? []).map((o) => (
			...
			return (
				<div key={field.key}>
					...
					<TextInput
						...
						value={String(resource!.values[field.key] ?? '')}
						onChange={(e) =>
							updateResourceValue(resource!.id, field.key, e.target.value === '' ? '' : Number(e.target.value))
						}
					/>
					{field.description && <Hint>{field.description}</Hint>}
				</div>
		...
		if (field.type === 'env_list') {
			return (
				<EnvVarsEditor
					key={field.key}
					value={resource!.values[field.key]}
					onChange={(next) => updateResourceValue(resource!.id, field.key, next)}
				/>
			);
		}
		...
		if (field.type === 'list') {
			const list = Array.isArray(resource!.values[field.key]) ? (resource!.values[field.key] as string[]) : [];
			return (
				<div key={field.key}>
					<Label
						htmlFor={field.key}
		...
		if (field.type === 'tags') {
			const tags =
				typeof resource!.values[field.key] === 'object' && resource!.values[field.key] !== null
			...
			return (
				<div key={field.key}>
					...
					<TextInput
						...
						autoComplete="off"
						value={String(resource!.values[field.key] ?? '')}
						onChange={(e) => updateResourceValue(resource!.id, field.key, e.target.value)}
						placeholder={field.placeholder ?? 'Will become a Terraform variable'}
					/>
					<Hint>{field.description || 'Emitted as a sensitive variable — never hardcoded in the export.'}</Hint>
				</div>
			);
		}
		...
		<Card className="p-5 flex flex-col h-full min-h-0 overflow-y-auto">
			...
			<div className="flex items-start justify-between gap-3 mb-4">
				<div>
					<div className="flex items-center gap-2 mb-1">
						...
						<h2 className="text-lg font-semibold text-slate-900 dark:text-slate-100">{def.label}</h2>
						{resource.useExisting ? <Badge tone="amber">existing</Badge> : <Badge tone="emerald">create</Badge>}
					</div>
					{resource.useExisting && (
			...
			<div className="space-y-4 mb-5">
				{resource.useExisting && (
					<div>
						...
						<TextInput
							...
							onChange={(e) => {
								const cleaned = e.target.value.replace(/[^a-zA-Z0-9_]/g, '_').replace(/^(\d)/, '_$1');
								updateResource(resource.id, { tfName: cleaned || 'app' });
							}}
						/>
						{state.resources.some(
							(r) => r.id !== resource.id && r.type === resource.type && r.tfName === resource.tfName,
						) && (
							<p className="mt-1 text-xs text-rose-600">
								Duplicate local name — another {def.label} already uses &quot;{resource.tfName}&quot;. Rename to avoid
								collisions.
							</p>
						)}
						<Hint>
							Short id used when wiring references
							{resource.type === 'azurerm_container_app' && ' — add more from the catalogue (app, app_2, …)'}.
						</Hint>
					</div>
				)}
				...
				<div className="rounded-lg border border-slate-200 dark:border-slate-700 p-3 space-y-2">
					<Label>Scope</Label>
					<SelectInput
						value={resource.scope?.kind === 'environment' ? resource.scope.environmentId : 'shared'}
						onChange={(e) => {
							const v = e.target.value;
							setResourceScope(resource.id, v === 'shared' ? sharedScope() : envScope(v));
						}}>
					...
					<Hint>
						{resource.type === 'azurerm_private_dns_zone_virtual_network_link' ? (
							<>
								VNet DNS links are usually <strong>Shared</strong> (hub networking). Ownership is separate from Prefer
								Existing — the owner chip stays until you explicitly reassign.
							</>
						) : resource.type === 'azurerm_private_dns_zone' ? (
							<>
								Shared + Existing is the hub DNS pattern. Prefer Existing does not move ownership; other Environments
								can reuse this zone.
							</>
						) : (
							<>
								Shared resources appear in every environment view. Env-scoped resources only appear in their
								environment. Refs cannot cross environments.
							</>
						)}
					</Hint>
				</div>
				...
				{isHubDnsType(resource.type) && (
					<HubOwnershipPanel
						...
						onOpenReassign={() => {
							const currentOwner = resource.hubOwnerEnvironmentId ?? state.activeEnvironmentId;
							const fallback =
								state.environments.find((e) => e.id !== currentOwner)?.id ?? state.environments[0]?.id ?? null;
			...
			{resource.useExisting ? (
				<div className="space-y-4 mb-5 rounded-lg border border-amber-200 dark:border-amber-800 bg-amber-50/50 dark:bg-amber-950/20 p-4">
					...
					<p className="text-xs text-amber-800 dark:text-amber-200 -mt-2 mb-2">
						These values look up the resource in Azure. Terraform will not create or destroy it.
					</p>
					{existingFields.map((f) => renderField(f))}
					{existingFields.length === 0 && (
						<p className="text-sm text-slate-500">This resource type has no identifying fields configured.</p>
					)}
				</div>
			) : (
				<>
					...
					<div className="mb-4">
						<button
							...
							{showAdvanced ? '▾ Hide advanced' : '▸ Show advanced'} ({advancedFields.length + 1})
						</button>
						{showAdvanced && (
							<div className="space-y-4 rounded-lg border border-slate-200 dark:border-slate-700 p-4">
								<div>
									...
									{state.resources.some(
										...
										<p className="mt-1 text-xs text-rose-600">
											Duplicate local name — another {def.label} already uses &quot;{resource.tfName}&quot;. Rename to
											avoid collisions.
										</p>
									)}
									<Hint>
			...
			<div className="mt-auto pt-4 border-t border-slate-200 dark:border-slate-700">
				...
				<div className="grid gap-3 sm:grid-cols-2 text-sm">
					<div>
						<div className="text-xs font-medium text-slate-500 mb-1.5">Depends on</div>
						{deps.length === 0 ? (
					...
					<div>
						<div className="text-xs font-medium text-slate-500 mb-1.5">Used by</div>
						{usedBy.length === 0 ? (
							<span className="text-xs text-slate-400">None</span>
						) : (
							<ul className="space-y-1">
