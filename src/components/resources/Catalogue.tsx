...
import { RESOURCE_BUILD_STAGES, resourceBuildStageForType, resourceTypeBuildOrder } from '@/lib/generate/modules';
import { useProject } from '@/lib/store/project-context';
import { Card, SectionTitle, TextInput, Badge, Label, Button } from '@/components/ui/Field';
...
export function Catalogue({
	...
	const filtered = useMemo(() => {
		...
		return [...matching].sort((a, b) => resourceTypeBuildOrder(a.type) - resourceTypeBuildOrder(b.type));
	}, [query]);
		...
		<>
			...
			<div className="flex-1 overflow-y-auto space-y-4 pr-1 min-h-0">
				{RESOURCE_BUILD_STAGES.map((stage, stageIndex) => {
					const items = filtered.filter((r) => resourceBuildStageForType(r.type)?.id === stage.id);
				...
				{filtered.length === 0 && (
					<p className="text-sm text-slate-400 text-center py-6">No resources match “{query}”</p>
				)}
			</div>
		</>
	);
