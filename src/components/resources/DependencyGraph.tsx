...
import { hubOwnershipBadgeText, hubOwnershipGraphSubtitle } from '@/lib/store/hub-dns-ownership';
	...
	return (
		<Card className="p-4 flex flex-col h-full min-h-0 relative">
			...
			<p className="text-[11px] text-slate-400 mb-2">
				Showing shared + {activeEnv?.displayName ?? 'env'} · {sharedCount} shared · {scopedCount} scoped · edges from
				Environment refs (cross-env blocked omitted)
						...
						{layout.edges.map((e) => {
							...
							return (
								<path
									...
									className={
										dim ? 'stroke-slate-300 dark:stroke-slate-600 opacity-40' : 'stroke-slate-400 dark:stroke-slate-500'
									}
									aria-hidden
								/>
							);
						...
						{layout.nodes.map((n) => {
							...
							return (
								<g
									...
									{(() => {
										const sub = hubOwnershipGraphSubtitle(r, resources, environments);
										if (!sub) return null;
										const short = sub.length > 26 ? sub.slice(0, 25) + '…' : sub;
										return (
											<text
												x={12}
												y={54}
									...
									<text
										...
										className={
											r.useExisting ? 'fill-amber-800 dark:fill-amber-200' : 'fill-emerald-800 dark:fill-emerald-200'
										}>
										{r.useExisting ? 'exist' : 'create'}
									</text>
									<title>
										{label} .{r.tfName} · {r.useExisting ? 'Existing' : 'Create'} ·{' '}
