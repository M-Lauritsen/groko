...
Generated resource blocks follow a dependency-aware, human-readable order: foundation and networking, shared storage and security, app platform prerequisites, identities and RBAC, workloads, then private networking integration. Terraform does not apply these blocks sequentially; references determine its dependency graph.
...
export function generateProject(
  ...
  for (const mid of orderedModules) {
    const modResources = sortResourcesByBuildOrder(partitioned.get(mid) ?? []);
  ...
  files["README.md"] = generateProjectReadme(
    config,
    sortResourcesByBuildOrder(resources),
    uniqueSensitive,
    orderedModules
  );
