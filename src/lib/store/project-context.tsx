...
export function ProjectProvider({ children }: { children: React.ReactNode }) {
	const [history, setHistory] = useState<HistoryStack<ProjectState>>(() => createHistory(initialState));
	...
	const commit = useCallback(
		(mutator: (s: ProjectState) => ProjectState) => {
			...
			setHistory((h) => mutateWithHistory(h, mutator, { clone: cloneProjectState }));
		},
		[clearValueEditCoalesce],
	);
	...
	const getUniqueTfName = useCallback(
		(type: string, preferred?: string) => {
			...
			const existing = new Set(state.resources.filter((r) => r.type === type).map((r) => r.tfName));
	...
	const replaceProject = useCallback((project: ProjectState | string) => {
		const nextState = typeof project === 'string' ? parseProjectFile(project) : project;
		setHistory(createHistory(cloneProjectState(nextState)));
	}, []);
	...
	const removeEnvironment = useCallback(
		(id: string) => {
			commit((s) => {
				...
				const activeEnvironmentId = s.activeEnvironmentId === id ? environments[0].id : s.activeEnvironmentId;
				// Re-scope resources that pointed at the removed env → shared
				const resources = s.resources.map((r) => {
					if (r.scope?.kind === 'environment' && r.scope.environmentId === id) {
	...
	const addResource = useCallback(
		(type: string) => {
			...
			commit((s) => {
				const existingNames = new Set(s.resources.filter((r) => r.type === type).map((r) => r.tfName));
				...
				for (const f of def.fields) {
					if (f.defaultValue !== undefined) {
						values[f.key] = f.type === 'tags' ? { ...s.config.tags } : f.defaultValue;
					}
				}
				...
				if (values.name === '' || values.name === undefined) {
					...
					const suffix =
						tfName === 'main' || tfName === 'app' || tfName === 'default' ? '' : `-${tfName.replace(/_/g, '-')}`;
					...
					if (type === 'azurerm_container_app') {
						values.name = prefix ? `${prefix}-ca-${tfName.replace(/_/g, '-')}` : `ca-${tfName.replace(/_/g, '-')}`;
					}
				}

				if (type === 'azurerm_container_app') {
					const env = s.resources.find((r) => r.type === 'azurerm_container_app_environment');
					if (env) {
					...
					const acr = s.resources.find((r) => r.type === 'azurerm_container_registry');
					if (acr) {
					...
					const uai = s.resources.find((r) => r.type === 'azurerm_user_assigned_identity');
					if (uai) {
				...
				if (type === 'azurerm_container_app_environment') {
					const caeSubnet = s.resources.find(
						(r) => r.type === 'azurerm_subnet' && r.values.delegation === 'Microsoft.App/environments',
					);
					if (caeSubnet) {
					...
					const law = s.resources.find((r) => r.type === 'azurerm_log_analytics_workspace');
					if (law) {
				...
				if (rg) {
					for (const f of def.fields) {
						if (f.type === 'reference' && f.refTypes?.includes('azurerm_resource_group')) {
							values[f.key] = {
								resourceId: rg.id,
								attr: f.refAttr ?? 'id',
							};
	...
	const updateResourceValue = useCallback(
		(id: string, key: string, value: unknown) => {
			commitValueEdit((s) => ({
				...s,
				resources: s.resources.map((r) => (r.id === id ? { ...r, values: { ...r.values, [key]: value } } : r)),
			}));
		},
		[commitValueEdit],
	);
	...
	const setResourceScope = useCallback(
		(id: string, scope: ResourceScope) => {
			commit((s) => ({
				...s,
				resources: s.resources.map((r) => (r.id === id ? { ...r, scope } : r)),
			}));
		},
		[commit],
	);
	const reassignHubOwnerEnvironment = useCallback(
		(id: string, environmentId: string) => {
			commit((s) => ({
				...s,
				resources: s.resources.map((r) => (r.id === id ? reassignHubOwner(r, environmentId) : r)),
			}));
		},
		[commit],
	);
		...
		(resources: ResourceInstance[], mode: 'merge' | 'replace') => {
			commit((s) => {
				...
				const selectId =
					mode === 'replace'
						? (next[0]?.id ?? null)
						: (next[Math.max(0, next.length - resources.length)]?.id ?? next[0]?.id ?? null);
				return {
					...s,
					config: {
						...s.config,
					...
					exportConfig: mode === 'replace' ? defaultExportConfig() : pruneExportConfig(s.exportConfig, next),
				};
			});
		},
		[commit],
	...
	const setTypeGroupModule = useCallback(
		(type: string, moduleId: string | null) => {
			commit((s) => ({
				...s,
				exportConfig: withTypeGroupModule(s.exportConfig, s.resources, type, moduleId),
			}));
		},
		[commit],
	);
	...
	const setDomainGroupModule = useCallback(
		(fromModuleId: string, toModuleId: string | null) => {
			commit((s) => ({
				...s,
				exportConfig: withDomainGroupModule(s.exportConfig, s.resources, fromModuleId, toModuleId),
			}));
		},
		[commit],
	);
