...
4. **List | Graph** — Tab to the Resources view toggle; switch to **Graph**. Nodes are shared + active-env resources (Existing/Create + Shared vs env styling). Arrow/Tab to a node; **Enter** selects — the same **ResourceForm** (mode, scope, fields, deps/used-by) opens in the side panel (one selection model). On Graph, Tab to **+ Add** (aria-label **Add from catalogue**); Enter opens the catalogue drawer (Esc closes). Add a type — stay on Graph; the new node is selected and ResourceForm updates (shared selection). Switch back to **List**; selection stays. Cross-env-blocked refs are not drawn as valid edges. Undo still works.
